...
const STRINGS = {
  ...
  'deadline.over': ['마지막 요청은 기억을 준비하는 데 제한 시간 {d}ms를 넘겨{took} 기억 없이 보냈습니다. 설정 탭에서 제한 시간(ms)을 {s} 정도로 올려 보세요. 늘린 만큼 답장 시작이 늦어질 수 있습니다.',
  ...
  'deadline.near': ['마지막 요청은 제한 시간 {d}ms 중 {n}ms를 썼습니다. 채팅이 더 길어지면 기억이 빠질 수 있으니, 설정 탭에서 제한 시간(ms)을 {s} 정도로 올려 두세요.',
  ...
  'deadline.alert': ['NMOS: 이번 답장은 기억 없이 보냈습니다. 기억 준비가 제한 시간 {d}ms를 넘겼습니다{took}. NMOS 패널 → 설정 탭 → 제한 시간(ms)을 {s} 정도로 올려 보세요. (이 알림은 페이지를 새로 열 때까지 다시 뜨지 않습니다.)',
  ...
  'link.sub': ['이야기가 스스로 잇지 못한 이름을 직접 합칩니다(예: 이름 없이 먼저 나온 인물과 나중에 이름이 밝혀진 인물). 기억 재구축을 해도 유지되고, 언제든 해제할 수 있습니다.',
  ...
  'hud.reason.deadline': ['제한 시간 {s}초 초과 · 눌러서 늘리기', 'over the {s} s deadline · tap to raise'],
