...
// build with one SHA-256 per message, and reconcile payload serialization. Node, not a browser:
...
const { buildManifest } = await import(pathToFileURL(file).href);
...
console.log('| Messages | snapshot copy p50 | manifest + SHA-256 p50 / p95 | JSON p50 | reconcile payload |');
console.log('|---:|---:|---:|---:|---:|');
for (const n of sizes) {
  const source = chat(n);
  const copy = [], manifest = [], json = [];
  let bytes = 0;
  ...
  console.log(`| ${n.toLocaleString('en')} | ${f(pct(copy, 0.5))} | ${f(pct(manifest, 0.5))} / ${f(pct(manifest, 0.95))} | ${f(pct(json, 0.5))} | ${(bytes / 1024 / 1024).toFixed(2)} MB |`);
}
