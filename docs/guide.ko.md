...
- **인스펙터**: 인스펙터를 NMOS 화면 안에서 보여 줍니다(PocketRisu는 플러그인이 브라우저 탭을 여는 것을
  ...
  있습니다. 대화 화면 위쪽 버튼 두 개는 그 채팅에만 적용됩니다.
  - **과거 전체 추출**: 처음 연결할 때 건너뛴 이전 턴까지 사실을 추출하고 임베딩합니다.
  - **기억 재구축**(두 번 누름): 이 채팅의 사실을 버리고 모든 턴을 다시 추출합니다. 끝날 때까지 사실이 비어
    있을 수 있습니다.
  - 원문 메시지는 건드리지 않습니다. 둘 다 백그라운드에서 돌고, 턴마다 LLM 호출 1회 비용이 듭니다.
- **설정**
