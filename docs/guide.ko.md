...
않습니다(`NMOS_PACKET_POLICY=packet-v0`이면 예전 방식).
...
- 아주 긴 채팅: PocketRisu v1.12.0에서 측정하니 답장이 시작되기 전 NMOS가 메시지 5,000개에서 약
  ...
  잠시 멈춤). 기본 제한 시간 3초로 약 10,000개까지 기억이 붙습니다. 단, 사실·벡터가 없는 채팅으로 잰
  수치라 추출과 임베딩을 켜면 그만큼 더 걸립니다(호스트에서는 재지 않음). 더 긴 채팅이나 기억이 빠지는
  긴 채팅은 패널 설정 탭의 **제한 시간(ms)**을 늘리세요(15,000개면 약 5,000). 늘리지 않으면 그 요청은
  기억 없이 진행됩니다(채팅은 정상). 자세한 수치는 `docs/perf/scale.md`.
