...
- 아주 긴 채팅: PocketRisu v1.12.0에서 측정하니 메시지 5,000개에서 약 1.5초, 10,000개에서 약 2.7초가
  걸렸습니다(호스트가 채팅 복사본을 넘겨준 뒤 잠시 멈춤). 기본 마감 시간(800 ms)에서는 이런 요청이
  기억 없이 진행됩니다(채팅은 정상). 플러그인 `deadline_ms`를 늘리면 완화됩니다. 자세한 수치는
  `docs/perf/scale.md`.
