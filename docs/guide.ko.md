...
채팅 입력창 왼쪽의 **☰ 메뉴 → NMOS 기억**, 또는 PocketRisu → 설정 → **NMOS 상태** / **NMOS 설정**을 누르면
NMOS 화면이 열립니다. 위쪽 탭으로 **상태**와 **설정**을 오가고, 오른쪽 위에서 **한국어/English**를 고릅니다
(기본값 한국어, 메뉴 이름은 PocketRisu를 새로 고친 뒤 바뀝니다).

- **상태**: 사이드카 연결, 기능(상태창·사실 추출·의미 검색) 켜짐 여부, 마지막 요청에서 넣은 기억,
  인스펙터 열기
- **설정**
  - **연결**: 사이드카 주소, 경로, 기억 예산, 제한 시간, 켜기/끄기
...
- 아주 긴 채팅: 기본 마감 시간(800 ms) 안에 기억이 붙는 것은 약 5,000개 메시지까지 측정했습니다.
  약 8,000개가 넘으면 대부분의 요청이 시간 초과로 기억 없이 진행됩니다(채팅은 정상).
  플러그인 `deadline_ms`를 늘리면 완화됩니다. 자세한 수치는 `docs/perf/scale.md`.
- LLM·임베딩 모델이나 주소를 바꾸면 예전에 처리했던 대화를 새 모델로 다시 처리합니다(최근 메시지 먼저).
  그동안 인스펙터에 "partial"로 표시되며, 원격 유료 모델이라면 그만큼 비용이 듭니다.
