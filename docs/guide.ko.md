...
- **설정**
  ...
  - **Google Vertex AI**(사실 추출 LLM만): 서비스 계정 JSON 키 파일 내용을 API 키 칸에 통째로 붙여 넣으세요.
    ...
    전용 서비스 계정을 쓰세요. 모델 목록 불러오기는 Vertex에서 동작하지 않을 수 있으니 모델 이름을 직접 적으세요.
...
채팅 화면 오른쪽 가운데에 작은 표시가 떠서, 답장마다 기억이 들어갔는지와 이 채팅의 백그라운드 처리가 어디까지
...
메뉴 → **권한 응답 초기화** 후 다시 켜세요. 켜기 아래의 **위치**에서 네 모서리 중 하나로 옮길 수 있습니다
(오른쪽 위는 PocketRisu의 요청 상태 알림과 플러그인 버튼이 쌓이는 자리라 가려질 수 있습니다).

| 표시 | 뜻 |
|---|---|
| `🧠 기억 불러오는 중…` | 이번 요청에 넣을 기억을 준비하는 중 |
| `✓ 기억 주입 (N자)` / `– 관련 기억 없음` | 기억을 넣음 / 관련 기억 없음 (4초) |
| `⚠ 건너뜀: 제한 시간 초과` | 이번 요청은 기억 없이 보냄 (제한 시간 또는 사이드카 오류) |
...
- **중요한 사건 먼저** (0.1.0-beta.14부터, Phase 7): 기억 패킷의 사실 중 사건은 최대 3개입니다. 고백·배신·죽음·비밀 폭로 같은
  ...
  메시지가 그 일에 대한 것일 때만 넣습니다. 사소한 사건도 지우지 않고 인스펙터에 남습니다.
