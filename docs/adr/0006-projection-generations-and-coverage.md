...
6. **Upgrade adoption.** Existing embedding rows become `legacy:<model>`. They are adopted into the
   first embedding projection ever registered when its model matches, because they were produced with
   the same normalizer and chunker; their endpoint was never recorded. Old extractions keep
   `extractor_key = NULL`: they are auditable but inactive, and step 4 rebuilds them.
