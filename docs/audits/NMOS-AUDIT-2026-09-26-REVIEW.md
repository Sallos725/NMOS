...
- Outcome: A-02 and A-04 are fixed on branch `audit-fixes-a02-a04` (this document's PR). Everything else
  is open and listed under [Open items](#open-items).

## Verdict

The audit is sound. No claim it marks as verified turned out wrong. Its overall rating (late beta, not
production-ready) stands. The corrections below are about severity, a narrower trigger for A-03, and a
cheaper fix for A-01.

## Claims checked

| ID | Result | Evidence |
|---|---|---|
| A-01 | **[verified] reproduced** | A throwaway test posting `"hello \ud800 there"` got reconcile 200 `needs_bodies`, then `UnicodeEncodeError … surrogates not allowed` from `/v1/sync/bodies`. Why it gets that far: pydantic's own JSON parser rejects a lone surrogate, but FastAPI 0.141 decodes bodies with `request.json()` (`json.loads`), which accepts it, and pydantic then validates a Python `str`. |
...
## A-01: a fix that keeps the hash contract
...
No schema, generation or hash change. At the next release, the plugin fix needs the new
`dist/nmos-pocketrisu.js` installed and the page reloaded (H13).
...
| Order | ID | What | Needs |
|---|---|---|---|
| 1 | A-01 | Option (c) above, with an ADR; listed as K27 until then | owner decision |
| 2 | A-03 | Pass `NMOS_LLM_JSON_MODE`, `NMOS_EXTRACT_HINTS` and `NMOS_LLM_API_KEY` to both dev services and `NMOS_EXTRACT_HINTS` to the release compose; warn in `worker.maintenance` when a handler key differs from the active generation | — |
| 3 | A-07 | Fix the doc drifts; extend `tools/check_release.py` to cross-check the H/D/K maxima and the phase file list | — |
| 4 | A-09 | Re-measure K1's 10k margin with extraction and embeddings on, or narrow the wording | real-host session |
| 5 | A-16 | Schema-transition test (0013 → 0020 with rows) and a `pg_dump` backup/rollback note | — |
| 6 | A-08 | Commit startup steps one by one | — |
| — | A-05, A-06, A-10, A-12, A-14, A-15 | As in the audit's owner-decision table | owner decision |

At release preparation, move A-02 and A-04 into `docs/KNOWN-ISSUES.md` → Resolved as "(not listed; audit
2026-09-26)".
